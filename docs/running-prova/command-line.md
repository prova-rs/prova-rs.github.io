...
The `prova` binary is a single command. Give it paths and it runs them; give it nothing and it runs the suite declared in `./prova.toml`. Every flag composes with either mode, and command-line flags always win over manifest values.
...
**Manifest-driven** runs kick in when you pass no paths. Prova reads `./prova.toml` (or the file given with `--manifest`), applies its environment variables, and runs its declared paths and suites:
...
This is the contract CI relies on: gate on the exit code, parse the output only when you want detail. The full flag-by-flag reference lives at [CLI Reference](../reference/cli.md).
